<?php
	...
	//Consulta
	$sql = "SELECT nombrevehiculo, fecha_registro, observacion FROM c_tipo_vehiculo WHERE estado='A'";
	...
	$objPHPExcel->getProperties()->setCreator("Fernando Reyes")->setDescription("Reporte de Tipo de Vehiculo");
	
	//Establecemos la pestaña activa y nombre a la pestaña
	$objPHPExcel->setActiveSheetIndex(0);
	$objPHPExcel->getActiveSheet()->setTitle("c_tipo_vehiculo");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE DE TIPO DE VEHICULO');
	...
	while($rows = $resultado->fetch_assoc()){
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['nombrevehiculo']);
	...
	$values = new PHPExcel_Chart_DataSeriesValues('Number', 'Vehiculo!$D$7:$D$'.$fila);
	
	// definir origen de los rotulos
	$categories = new PHPExcel_Chart_DataSeriesValues('String', 'Vehiculo!$B$7:$B$'.$fila);
	...
	header('Content-Disposition: attachment;filename="Vehiculo.xlsx"');
