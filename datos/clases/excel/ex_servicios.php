<?php
	...
	//Consulta
	$sql = "SELECT nombre_servicio, fecha_registro, observacion FROM c_servicios WHERE estado='A'";
	...
	$objPHPExcel->getProperties()->setCreator("Fernando Reyes")->setDescription("Reporte de Servicios");
	
	//Establecemos la pestaña activa y nombre a la pestaña
	$objPHPExcel->setActiveSheetIndex(0);
	$objPHPExcel->getActiveSheet()->setTitle("c_servicios");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE DE SERVICIOS');
	...
	while($rows = $resultado->fetch_assoc()){
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['nombre_servicio']);
	...
	$values = new PHPExcel_Chart_DataSeriesValues('Number', 'Servicios!$D$7:$D$'.$fila);
	
	// definir origen de los rotulos
	$categories = new PHPExcel_Chart_DataSeriesValues('String', 'Servicios!$B$7:$B$'.$fila);
