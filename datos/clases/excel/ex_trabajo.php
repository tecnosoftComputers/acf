<?php
	...
	//Consulta
	$sql = "SELECT nombretrabajo, fecha_registro, observacion FROM c_descripcion_trabajo";
	...
	$objPHPExcel->getProperties()->setCreator("Fernando Reyes")->setDescription("Reporte de Descripción de trabajo");
	
	//Establecemos la pestaña activa y nombre a la pestaña
	$objPHPExcel->setActiveSheetIndex(0);
	$objPHPExcel->getActiveSheet()->setTitle("c_descripcion_trabajo");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE DE DESCRIPCION DE TRABAJO');
	$objPHPExcel->getActiveSheet()->mergeCells('B3:I3');
	//nombretrabajo, fecha_registro, observacion
	$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(45);
	$objPHPExcel->getActiveSheet()->setCellValue('A6', 'NOMBRE TRABAJO');
	...
	while($rows = $resultado->fetch_assoc()){
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['nombretrabajo']);
	...
	$values = new PHPExcel_Chart_DataSeriesValues('Number', 'Trabajo!$D$7:$D$'.$fila);
	
	// definir origen de los rotulos
	$categories = new PHPExcel_Chart_DataSeriesValues('String', 'Trabajo!$B$7:$B$'.$fila);
