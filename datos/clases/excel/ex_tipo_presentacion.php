<?php
	...
	//Consulta
	$sql = "SELECT nombrepresentacion, fecha_registro, observacion FROM c_tipo_presentacion WHERE estado='A'";
	...
	$objPHPExcel->getProperties()->setCreator("Fernando Reyes")->setDescription("Reporte de Tipo de Presentacion");
	
	//Establecemos la pestaña activa y nombre a la pestaña
	$objPHPExcel->setActiveSheetIndex(0);
	$objPHPExcel->getActiveSheet()->setTitle("c_tipo_presentacion");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B3', 'REPORTE DE TIPO DE PRESENTACION');
	...
	while($rows = $resultado->fetch_assoc()){
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['nombrepresentacion']);
	...
	$values = new PHPExcel_Chart_DataSeriesValues('Number', 'Presentacion!$D$7:$D$'.$fila);
	
	// definir origen de los rotulos
	$categories = new PHPExcel_Chart_DataSeriesValues('String', 'Presentacion!$B$7:$B$'.$fila);
	...
	header('Content-Disposition: attachment;filename="Presentacion.xlsx"');
