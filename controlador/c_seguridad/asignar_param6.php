<?php
	function verPermisos6($cc, $param) {
    // M O D U L O      6
    $modulo1 = $cc->prepare("SELECT * FROM config WHERE id_config=6");
    $modulo1->execute();
    ...
    foreach((array) $all_modulo1 as $view1) {
        $namemode1 = $view1['name_access'];
                	...
                	$arg_mode1 = $cc->prepare("SELECT * FROM modulos_items WHERE modulo=3 AND acceso=6");
                	$arg_mode1->execute();
                	$datas1 = $arg_mode1->fetchAll(PDO::FETCH_ASSOC);
                	$count1 = $arg_mode1->rowCount();
                    
                    // Extrae los files del modulo 4
                	$arg_mode1b = $cc->prepare("SELECT * FROM modulos_items WHERE modulo=4 AND acceso=6");
                	$arg_mode1b->execute();
                	$datas1b = $arg_mode1b->fetchAll(PDO::FETCH_ASSOC);
                	$count1b = $arg_mode1b->rowCount();
                    
                    // Extrae los files del modulo 5
                	$arg_mode1c = $cc->prepare("SELECT * FROM modulos_items WHERE modulo=5 AND acceso=6");
                	$arg_mode1c->execute();
                	$datas1c = $arg_mode1c->fetchAll(PDO::FETCH_ASSOC);
                	$count1c = $arg_mode1c->rowCount();
                    
                    // Extrae los files del modulo 6
                	$arg_mode1d = $cc->prepare("SELECT * FROM modulos_items WHERE modulo=6 AND acceso=6");
                	$arg_mode1d->execute();
...
<div class="panel panel-default">
    <div class="panel-heading">
        <h4 class="panel-title">
            ...
            <p style="color: #fff; font-weight: bold"><i class="fa fa-file-text"></i> <?php echo $namemode1 ?><i class="fa fa-caret-down" style="float: right; color:#fff"></i></p></a>
        </h4>
    </div>
 ...
 <table class="table table-striped table-hover bg-dark">
    <thead >
        <th><i class="fa fa-unlock">Módulo</i> </th>
        <th><i class="fa fa-bullseye"></i> Consultar</th>
        <th><i class="fa fa-"></i> Agregar</th>
