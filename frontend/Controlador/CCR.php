<?php
class Controlador_CCR extends Controlador_Base {
  ...
  public function construirPagina(){
    ...
    switch($opcion){
        ...
        if(Utils::getParam('create') == 1){
          try{ 
            ...
            if(!Modelo_TabGeneral::insert($datos,$tabla)){
              throw new Exception('The credit classification could not be created, try again.');
            }
            $_SESSION['acfSession']['mostrar_exito'] = 'The credit classification was successfully created.';
            $GLOBALS['db']->commit();
          }
          catch(Exception $e){
            $GLOBALS['db']->rollback();
          ...
          Utils::doRedirect(PUERTO.'://'.HOST.'/ccreditList/');
        }

        $tags = array('type'=>'Create',
                      'view'=>$view);

        $tags["template_js"][] = "ccredit";
        $tags["template_css"][] = "";

        Vista::render('ccredit', $tags);  
      break;
      case 'update':
        ...
        if(Utils::getParam('save') == 1){
          try{ 
            ...
            if(!Modelo_TabGeneral::setUpdate($id,$datos,$tabla)){
              throw new Exception('The credit classification could not be edited, try again.');
            }
            $_SESSION['acfSession']['mostrar_exito'] = 'The credit classification was successfully edited.';
            $GLOBALS['db']->commit();
            Utils::doRedirect(PUERTO.'://'.HOST.'/ccreditList/');
          }
          catch(Exception $e){
        ...
        $ccredit = Modelo_TabGeneral::getUpdate($id,$tabla);
        $view = 'ccreditUpdate';
        $tags = array('rows'=>$ccredit,
                      'type'=>'Update',
                      'view'=>$view);
        ...
        if(Utils::getParam('save') == 1){
          try{ 
            ...
            if(!Modelo_TabGeneral::delete($id,$tabla)){
              throw new Exception('The credit classification could not be deleted, try again.');
            }
            $_SESSION['acfSession']['mostrar_exito'] = 'The credit classification was successfully delete.';
            $GLOBALS['db']->commit();
            Utils::doRedirect(PUERTO.'://'.HOST.'/ccreditList/');
          }
          catch(Exception $e){
      ...
      case 'report':
        ...
        $ccredit = Modelo_TabGeneral::search($tabla);
        $nombre_archivo = 'credit_classification_List';
        ...
        if($tipo == 'excel'){
          ...
          $objPHPExcel->getProperties()
            ...
            ->setTitle("credit_classification_List")
            ->setSubject("credit_classification_List")
            ->setCategory("credit_classification_List");
          ...
          $objPHPExcel->setActiveSheetIndex(0)
              ->setCellValue('A1', 'CREDIT CLASSIFICATION LIST');
          ...
          $pdf->Cell(59,5,'CREDIT CLASSIFICATION LIST',0,1);//end of line
          $pdf->Cell(189,10,'',0,1); //end of line
          $pdf->SetFont('Arial','B',12);
          $pdf->Ln(6);
