<?php
class Controlador_DES extends Controlador_Base {
  ...
  public function construirPagina(){
    ...
    switch($opcion){
        ...
        if(Utils::getParam('create') == 1){
          try{ 
            ...
            if(!Modelo_TabGeneral::insert($datos,$tabla)){
              throw new Exception('The destionation could not be created, try again.');
            }
            $_SESSION['acfSession']['mostrar_exito'] = 'The destionation was successfully created.';
            $GLOBALS['db']->commit();
          }
          catch(Exception $e){
            $GLOBALS['db']->rollback();
          ...
          Utils::doRedirect(PUERTO.'://'.HOST.'/creditDestsList/'); 
        }

        $tags = array('type'=>'Create',
                      'view'=>$view);

        $tags["template_js"][] = "creditDests";
        $tags["template_css"][] = "";

        Vista::render('creditDests', $tags);  
      break;
      case 'update':
        ...
        if(Utils::getParam('save') == 1){
          try{ 
            ...
            if(!Modelo_TabGeneral::setUpdate($id,$datos,$tabla)){
              throw new Exception('The destionation could not be edited, try again.');
            }
            $_SESSION['acfSession']['mostrar_exito'] = 'The destionation was successfully edited.';
            $GLOBALS['db']->commit();
            Utils::doRedirect(PUERTO.'://'.HOST.'/creditDestsList/');
          }
          catch(Exception $e){
        ...
        $creditDests = Modelo_TabGeneral::getUpdate($id,$tabla);
        $view = 'creditDestsUpdate';
        $tags = array('rows'=>$creditDests,
                      'type'=>'Update',
                      'view'=>$view);
        ...
        if(Utils::getParam('save') == 1){
          try{ 
            ...
            if(!Modelo_TabGeneral::delete($id,$tabla)){
              throw new Exception('The destionation could not be deleted, try again.');
            }
            $_SESSION['acfSession']['mostrar_exito'] = 'The destionation was successfully delete.';
            $GLOBALS['db']->commit();
            Utils::doRedirect(PUERTO.'://'.HOST.'/creditDestsList/');
          }
          catch(Exception $e){
      ...
      case 'report':
        ...
        $creditDests = Modelo_TabGeneral::search($tabla);
        $nombre_archivo = 'creditDests_List';
        ...
        if($tipo == 'excel'){
          ...
          $objPHPExcel->getProperties()
            ...
            ->setTitle("creditDests List")
            ->setSubject("creditDests List")
            ->setCategory("creditDests List");
          ...
          $objPHPExcel->setActiveSheetIndex(0)
              ->setCellValue('A1', 'CREDIT DESTINATIONS LIST');
          ...
          foreach ($creditDests as $key => $item) {
          ...
          $pdf->Cell(59,5,'CREDIT DESTINATIONS LIST',0,1);//end of line
          $pdf->Cell(189,10,'',0,1); //end of line
          $pdf->SetFont('Arial','B',12);
          $pdf->Ln(6);
      ...
      default:  
        ...
        $creditDests = Modelo_TabGeneral::search($tabla);
        $view = 'creditDestsCreate'; 
        $tags = array('creditDests'=>$creditDests,
                      'type'=>'Create',
                      'view'=>$view,
                      'error'=>$error);
        ...
        Vista::render('creditDestsList', $tags);
      break;
    }
