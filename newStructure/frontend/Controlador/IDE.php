<?php
class Controlador_IDE extends Controlador_Base {
  ...
  public function construirPagina(){
    ...
    switch($opcion){
      case 'create': 
        ...
        if(Utils::getParam('create') == 1){
          try{ 
            ...
            $_SESSION['mostrar_exito'] = 'The type was successfully created.';
            $GLOBALS['db']->commit();
          }
          catch(Exception $e){
            $GLOBALS['db']->rollback();
          ...
          Utils::doRedirect(PUERTO.'://'.HOST.'/typeIdentsList/'); 
        }

        $tags = array('type'=>'Create',
                      'view'=>$view);

        $tags["template_js"][] = "typeIdents";
        $tags["template_css"][] = "";

        Vista::render('typeIdents', $tags);  
      break;
      case 'update':
        ...
        if(Utils::getParam('save') == 1){
          try{ 
            ...
            $_SESSION['mostrar_exito'] = 'The type was successfully edited.';
            $GLOBALS['db']->commit();
            Utils::doRedirect(PUERTO.'://'.HOST.'/typeIdentsList/');
          }
          catch(Exception $e){
        ...
        $typeIdents = Modelo_TabGeneral::getUpdate($id,$tabla);
        $view = 'typeIdentsUpdate';
        $tags = array('rows'=>$typeIdents,
                      'type'=>'Update',
                      'view'=>$view);
      ...
      case 'delete':
        ...
        if(Utils::getParam('save') == 1){
          try{ 
            ...
            $_SESSION['mostrar_exito'] = 'The type was successfully delete.';
            $GLOBALS['db']->commit();
            Utils::doRedirect(PUERTO.'://'.HOST.'/typeIdentsList/');
          }
          catch(Exception $e){
      ...
      case 'report':
        ...
        $typeIdents = Modelo_TabGeneral::search($tabla);
        $nombre_archivo = 'typeIdents_List';
        ...
        if($tipo == 'excel'){
          ...
          $objPHPExcel->getProperties()
            ...
            ->setTitle("Type of Identification List")
            ->setSubject("Type of Identification List")
            ->setCategory("Type of Identification List");
          ...
          $objPHPExcel->setActiveSheetIndex(0)
              ->setCellValue('A1', 'TYPE OF IDENTIFICATION LIST');
          ...
          foreach ($typeIdents as $key => $item) {
          ...
          $pdf->Cell(59,5,'TYPE OF IDENTIFICATION LIST',0,1);//end of line
          $pdf->Cell(189,10,'',0,1); //end of line
          $pdf->SetFont('Arial','B',12);
          $pdf->Ln(6);
      ...
      default:  
        ...
        $typeIdents = Modelo_TabGeneral::search($tabla);
        $view = 'typeIdentsCreate'; 
        $tags = array('typeIdents'=>$typeIdents,
                      'type'=>'Create',
                      'view'=>$view,
                      'error'=>$error);
        ...
        Vista::render('typeIdentsList', $tags);
      break;
    }
